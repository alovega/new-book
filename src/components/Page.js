...
class Page extends React.Component{
   constructor(props){
     ...
     this.state = {
       ...contentToRender,
       data: contentToRender.pages.slice(0, 2),
       maximumItems: contentToRender.pages.length,
       currentPage: 1,
       start: 0,
       stop: 2
     }
   ...
   changePage(direction) {
    ...
    this.setState({
      ...
      stop: this.state.stop  - 2,
      data: contentToRender.pages.slice(this.state.start - 2, this.state.stop - 2 )
    });
    ...
    render(){
      return(
        ...
        <PageToken book={this.state} />
        {this.state.currentPage > 1 ?
          <button onClick={() => this.changePage('back')}>Back</button>
        : null}
        {this.state.pages.length -1 > this.state.currentPage * 2 ?
